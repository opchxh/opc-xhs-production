...
def make_complete_episode(root: Path) -> dict:
    files = {
        ...
        "topic-angle.md": "选题方向：别再把 AI 当搜索框，要让 Agent 接管完整工作流。",
        "voiceover-analysis.md": "原稿问题：开头不够钩子，口语节奏弱。",
        "voiceover-script.md": "新版口播稿：三秒钩子，然后进入步骤。",
        "recording-guide.md": "录制提示：分段录音；真人只拍开头出镜、强调观点、总结收口。",
        "poster-titles.md": "海报主标题：别再手搓流程\n海报副标题：让 Skill 自动接力",
        "note-package.md": "主标题：OPC 小红书生产流\n副标题：从 MD 到成片\n内容：...\n标签：#AI #小红书",
        "recordings/01-hook.m4a": "dry-run audio placeholder",
        "clips/opening.mov": "dry-run video placeholder",
        "assets/screenshot.png": "dry-run auxiliary placeholder",
        "animation-brief.md": "动效方案：HyperFrames，三段结构，OPC 碎碎念探长标准剪辑风格，字幕安全区，渲染 QA。",
    ...
    return {
        ...
        "deliverables": {
            ...
            "audio_assets": ["recordings/01-hook.m4a"],
            "video_assets": ["clips/opening.mov"],
            "auxiliary_assets": ["assets/screenshot.png"],
            "recording_assets": ["recordings/01-hook.m4a", "clips/opening.mov", "assets/screenshot.png"],
